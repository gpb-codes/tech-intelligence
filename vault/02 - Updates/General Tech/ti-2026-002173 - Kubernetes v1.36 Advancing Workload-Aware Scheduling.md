...
original_title: 'Kubernetes v1.36: Advancing Workload-Aware Scheduling'
company: ''
product: ''
version: '1.36'
date: '2026-05-13'
created: '2026-05-13T18:35:00+00:00'
updated: '2026-08-19T17:51:04+00:00'
...
>
> *   **New Features:**
>     *   Gang Scheduling: permite prevenir múltiples Pods de la misma Node simultáneamente.
>     *   PodGroup Priority: permite priorizar Pods dentro de un PodGroup.
>     *   PodGroup Disruption Mode: determina si los Pods dentro de un PodGroup pueden ser preemptados independientemente o en un modo all-or-nothing.
>     *   DRA ResourceClaim Support: permite a los Pods hacer solicitudes detalladas de dispositivos como GPUs, TPUs y NICs.
>     *   Workload API Integración con el Controlador de Jornadas: permite la creación automática de objetos de Jornada y PodGroup a través de la API de trabajo.
> *   **Improvements:**
>     *   Gradual Release: se están implementando características nuevas como Gang Scheduling y Multi-Level Workload Hierarchies.
>     *   Unified Controller Integration API: se está desarrollando una API para que los controles de trabajo puedan consumir las capacidades de trabajo-aware scheduling.
> *   **Next Steps:**
>     *   Pruebas y retroalimentación: se anima a la comunidad a probar las nuevas características y proporcionar retroalimentación para ayudar a definir el futuro de la gestión de carga de trabajo en Kubernetes.
>     *   Graduación a Beta: se están gradualmente implementando las nuevas características para lograr la madurez en v1.37.
>     *   Desarrollo de características adicionales: se están trabajando en características adicionales, como soporte para Jornadas Elastic y otros controles de carga de trabajo.

## ¿Qué ocurrió? ⚔️

> [!info] Traducción del anuncio
>
> The article discusses the new features and improvements in Kubernetes v1.36, specifically in the area of workload-aware scheduling. Here are the key points:
> 
> **New Features:**
> 
> 1. **Gang Scheduling**: The scheduler now uses a new mechanism to try making a scheduling of a PodGroup possible, treating the entire PodGroup as a single preemptor unit. This allows for simultaneous preemption of multiple Pods from the same Node.
> 2. **PodGroup Priority**: The PodGroup API now includes a `priority` field that overrides the priority of individual Pods forming the PodGroup.
> 3. **PodGroup Disruption Mode**: The PodGroup API now includes a `disruptionMode` field that determines whether Pods within a PodGroup can be preempted independently or in an all-or-nothing fashion.
> 4. **DRA ResourceClaim Support**: The `DRA` (Device Requested Allocation) feature now enables Pods to make detailed requests for devices like GPUs, TPUs, and NICs. Requested devices can be shared by multiple Pods requesting the same ResourceClaim.
> 5. **Workload API Integration with Job Controller**: The Job controller now automates the creation of Workload and PodGroup objects, allowing for gang scheduling and other workload-aware scheduling features.
> 
> **Improvements:**
> 
> 1. **Gradual Release**: The new features are available as Alpha features in v1.36, with the goal of gradually rolling them out to production.
> 2. **Multi-Level Workload Hierarchies**: The architecture is now designed to support complex modern AI workloads like JobSet or Disaggregated Inference via LeaderWorkerSet (LWS).
> 3. **Unified Controller Integration API**: A new API is being developed to provide a unified method for workload controllers to consume workload-aware scheduling capabilities.
> 
> **Next Steps:**
> 
> 1. **Testing and Feedback**: The community is encouraged to try out the new features and provide feedback to help shape the future of Kubernetes scheduling.
> 2. **Graduation to Beta**: The new features will be gradually rolled out to production, with the goal of achieving maturity in v1.37.
> 3. **Future Releases**: The community is working on additional features, including support for elastic Jobs and other workload controllers.
...
> - New features
> - Gradual release
> - Multi-level workload hierarchies
...
- **Docker Swarm** — confianza: medium
- **Kubernetes on AWS** — confianza: high
- **OpenShift** — confianza: medium
- **Red Hat OpenShift Container Platform** — confianza: high
- **Google Kubernetes Engine** — confianza: high
- **Microsoft Azure Kubernetes Service** — confianza: high

## Fuente original 📜

[https://kubernetes.io/blog/2026/05/13/kubernetes-v1-36-advancing-workload-aware-scheduling/](https://kubernetes.io/blog/2026/05/13/kubernetes-v1-36-advancing-workload-aware-scheduling/)
...
[[ti-2026-002173 - original|📄 Contenido original completo]]
